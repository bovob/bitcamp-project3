...
//        returnCommand = new ReturnCommand( bookCommand.getBookList(), bc.getBorrowList(), UserCommand.getCurrentUser());
    ...
    public static List<Book> addDummyBook() {
        ...
        String[][] dummy = {
                {"상식적으로 상식을 배우는 법", "제바스티안 클루스만", SEL, "istj"},
                {"부자의 언어", "존 소포릭", SEL, "istj"},

                {"기분을 관리하면 인생이 관리된다", "김다슬", SEL, "isfj"},
                {"열한계단", "채사장", PHI, "isfj"},
                ...
                {"역사가 묻고 화학이 답하다", "장홍제", SCI, "istp"},
                {"사람을 움직이는 처세술","데일 카네기",SEL,"istp"},

                {"창조적 사고의 놀라운 역사", "슈테판 클라인", HIS, "isfp"},
                ...
                {"나는 나를 고용하기로 했다", "토머스 오퐁", SEL, "estp"},
                {"Demian","헤르만헤세", NOV, "estp"},

                {"힘들어도 사람한테 너무 기대지 마세요", "정우열", SEL, "esfp"},
                {"포노사피엔스","최재봉",SCI, "esfp"},
